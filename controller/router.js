...
//发表评论
exports.fapinglun = function(req,res) {
    var form = new formidable.IncomingForm();
    form.parse(req, function (err, fields) {
        var email = fields.email;
        ...
        tool.fapinglun(email,postid,content,function(result){
            res.send(result); //1成功，-1不成功
        });
    });
}
//得到评论
exports.getPinglun = function(req,res){
    ...
    Post.find({"_id" : postid},function(err,posts){
        ...
        //所有评论的_id数组
        var allpinglunarray = posts[0].comments;

        //最终返回的数组
        var RESULT = [];
        var count = 0;
        for(var i = 0  ; i < allpinglunarray.length ; i++){
            (function(i){
                Comment.find({"_id" : allpinglunarray[i]},function(err,pingluns){
                    //这条评论
                    var p = pingluns[0];
                    //在此发出对User的请求
                    User.find({"email": p.email},function(err,users){
                        var u = users[0];
                        //放入字典：
                        RESULT.push({
                            "pinglunrennickname" : u.nickname,
                            "pinglunshijian" : p.date,
                            "pinglunrentouxiang" : u.avatar,
                            "pinglunneirong" : p.content,
                            "pinglunrenyouxiang" : u.email,
                            "pinglunrenjianjie" : u.signature
                        });

                        count++;
                        if(count == allpinglunarray.length){
                            res.send(RESULT);
                        }
                    });
                });
